...
skew_theory <- -(lambda_john^3 * exp(-delta_john^-2/2) * (exp(delta_john^-2) - 1)^2 *
                   ((exp(delta_john^-2) * (exp(delta_john^-2) + 2)) * sinh(3 * gamma_john / delta_john) +
                      3 * sinh(2 * gamma_john / delta_john))) / 4

term <- exp(delta_john^-2)
K1 <- term^2 * (term^4 + 2 * term^3 + 3 * term^2 - 3) * cosh(4 * gamma_john / delta_john)
K2 <- 4 * term^2 * (term + 2) * cosh(2 * gamma_john / delta_john)
K3 <- 3 * (2 * term + 1)

kurt_theory <- (lambda_john^4 * (term - 1)^2 * (K1 + K2 + K3)) / (8 * (var_theory)^2)
